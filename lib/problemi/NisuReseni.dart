...
import 'ReseniProblemi.dart';
/*
class NisuReseni extends StatefulWidget {
  @override
  _NisuReseniState createState() => _NisuReseniState();
}

class _NisuReseniState extends State<NisuReseni> {
  List<Post> postovi = [];
  int idPoslednjeg;
  double x;
  double y;
 ...
 @override
  ...
  void fetch() async{
    ...
    try{
      response = await http.get("http://10.0.2.2:62054/post/unsolved/"+x.toString()+"/"+y.toString()+"/"+idPoslednjeg.toString());
    }
    catch(Exception){
      return;
    }
                  ...
                  child: IconButton(
                    ...
                    onPressed: () {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                            builder: (context) => ReseniProblemi()),
                      );
                    },
                  ),
                ),
